...
ROOT_SIZE=$(du -x --apparent-size -s "${EXPORT_ROOTFS_DIR}" \
    --exclude var/cache/apt/archives --exclude boot/firmware \
...
parted --script "${IMG_FILE}" unit B mkpart primary ext4 \
    "${DATA_PART_START}" "$((DATA_PART_START + DATA_PART_SIZE - 1))"
...
mkfs.ext4 -L cp0-data -m 1 -O "$ROOT_FEATURES" "$DATA_DEV" >/dev/null

mount -v "$ROOT_DEV" "${ROOTFS_DIR}" -t ext4
mkdir -p "${ROOTFS_DIR}/boot/firmware"
mount -v "$BOOT_DEV" "${ROOTFS_DIR}/boot/firmware" -t vfat
mkdir -p "${ROOTFS_DIR}/var/lib/cardputerzero-persist"
mount -v "$DATA_DEV" "${ROOTFS_DIR}/var/lib/cardputerzero-persist" -t ext4

rsync -aHAXx --exclude /var/cache/apt/archives --exclude /boot/firmware \
    "${EXPORT_ROOTFS_DIR}/" "${ROOTFS_DIR}/"
rsync -rtx "${EXPORT_ROOTFS_DIR}/boot/firmware/" \
    "${ROOTFS_DIR}/boot/firmware/"

DATA_ROOT="${ROOTFS_DIR}/var/lib/cardputerzero-persist"
...
rsync -aHAXx "${ROOTFS_DIR}/var/lib/cardputerzero/" \
    "$DATA_ROOT/cardputerzero/"
rsync -aHAXx "${ROOTFS_DIR}/etc/cardputerzero/" \
    "$DATA_ROOT/etc-cardputerzero/"
rsync -aHAXx "${ROOTFS_DIR}/etc/ssh/" "$DATA_ROOT/ssh/"
if [[ -d ${ROOTFS_DIR}/etc/NetworkManager/system-connections ]]; then
    rsync -aHAXx "${ROOTFS_DIR}/etc/NetworkManager/system-connections/" \
        "$DATA_ROOT/network-connections/"
fi
...
chmod 0600 "$DATA_ROOT/random-seed"
sync -f "$DATA_ROOT"
