...
GitHub Release workflow 运行在公开的 `ubuntu-24.04-arm` runner 上，pi-gen 的
ARM64 阶段会原生执行，不再依赖速度较慢的 QEMU 模拟。workflow 仍保留 x86_64
回退分支，在启动特权 pi-gen 容器前向 runner 主机注册 `qemu-aarch64`；只在容器内
配置相关软件包是不够的。
