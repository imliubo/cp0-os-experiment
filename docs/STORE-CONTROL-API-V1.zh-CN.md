...
<!-- doc-locale: zh-CN -->
> [English](STORE-CONTROL-API-V1.md) | **简体中文**
...
content digest 固定为 SHA-256：先写入 ASCII domain
`CardputerZero Store submission content v1\0`，再按 package SHA、Listing SHA 分别写入
`u64 big-endian length + UTF-8 bytes`；随后按
...
只有自动扫描通过的 revision 可以进入 `READY_FOR_REVIEW`；只有 Review Service 可以把
revision 转换为 `IN_REVIEW`、`PENDING_SECONDARY_REVIEW`、`APPROVED`、`NEEDS_CHANGES` 或
`REJECTED`。主审批准只进入
