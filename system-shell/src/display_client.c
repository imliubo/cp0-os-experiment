...
#ifndef CP0_DISPLAY_SOCKET
#define CP0_DISPLAY_SOCKET "/run/cardputerzero-displayd/display.sock"
#endif

#define CP0_DISPLAY_FRAME_BYTES 2048U
#define CP0_DISPLAY_JSON_TOKENS 48U
#define CP0_DISPLAY_TIMEOUT_MSEC 500U
                    ...
                    size_t response_capacity, size_t *response_length)
{
    const struct timeval timeout = {
        .tv_sec = 0,
        .tv_usec = CP0_DISPLAY_TIMEOUT_MSEC * 1000U,
    };
    struct sockaddr_un address = {.sun_family = AF_UNIX};
    socklen_t address_length =
        (socklen_t)(offsetof(struct sockaddr_un, sun_path) +
                    strlen(CP0_DISPLAY_SOCKET) + 1U);
    ...
    if (fcntl(descriptor, F_SETFD, FD_CLOEXEC) != 0 ||
        ...
        setsockopt(descriptor, SOL_SOCKET, SO_SNDTIMEO, &timeout,
                   sizeof(timeout)) != 0 ||
        strlen(CP0_DISPLAY_SOCKET) >= sizeof(address.sun_path))
        goto cleanup;
    memcpy(address.sun_path, CP0_DISPLAY_SOCKET,
           strlen(CP0_DISPLAY_SOCKET) + 1U);
#ifdef __APPLE__
...
{
    struct cp0_json_token tokens[CP0_DISPLAY_JSON_TOKENS];
    int count = cp0_json_parse(document, length, tokens,
                               CP0_DISPLAY_JSON_TOKENS);
    uint64_t version;
    uint64_t parsed_id;
    int version_token;
    int id_token;
    ...
    if (cp0_json_string_equals(document, &tokens[status], "error")) {
        ...
        if (code >= 0 && cp0_json_string_equals(document, &tokens[code],
                                                 "unavailable"))
            return CP0_DISPLAY_UNAVAILABLE;
        return CP0_DISPLAY_FAILED;
    }
    ...
    int brightness_token = cp0_json_object_get(
        document, tokens, (size_t)count, state_token, "brightness_percent");
    bool available;
    if (available_token < 0 || brightness_token < 0 ||
        !cp0_json_get_bool(document, &tokens[available_token], &available))
        return CP0_DISPLAY_FAILED;
    if (!available) {
        if (!cp0_json_is_null(document, &tokens[brightness_token]))
            return CP0_DISPLAY_FAILED;
        *state = (struct cp0_display_state){0};
        return CP0_DISPLAY_UNAVAILABLE;
    }
    uint64_t percent;
    if (!cp0_json_get_u64(document, &tokens[brightness_token], &percent) ||
        percent > CP0_DISPLAY_BRIGHTNESS_MAX_PERCENT)
        return CP0_DISPLAY_FAILED;
    *state = (struct cp0_display_state){
        .available = true,
        .brightness_percent = (unsigned int)percent,
    };
    return CP0_DISPLAY_OK;
}
...
{
    char request[256];
    char response[CP0_DISPLAY_FRAME_BYTES];
                                  ...
                                  struct cp0_display_state *state)
{
    ...
    char member[64];
    int count;
    if (direction == CP0_DISPLAY_DECREASE)
        value = "decrease";
    else if (direction == CP0_DISPLAY_INCREASE)
        value = "increase";
    else
        return CP0_DISPLAY_FAILED;
    count = snprintf(member, sizeof(member), ",\"direction\":\"%s\"", value);
    if (count <= 0 || (size_t)count >= sizeof(member))
        return CP0_DISPLAY_FAILED;
    return command("adjust-brightness", member, state);
}

#ifdef CP0_DISPLAY_CLIENT_TEST
