...
grep -q 'pkg-config --cflags --libs wayland-client libpng libdrm' "$stage"
...
grep -q '^Wants=cardputerzero-provisiond.socket$' "$shell_service"
grep -q '^After=cardputerzero-compositor.service cardputerzero-provisiond.socket$' "$shell_service"
...
grep -q '^SupplementaryGroups=cp0-wayland cp0-control cp0-display-control cp0-audio-control cp0-connectivity-control cp0-provision-control$' "$shell_service"
...
grep -q '<interface name="cp0_system_shell_v1" version="6">' "$protocol"
