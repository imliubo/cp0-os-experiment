...
grep -q '/tmp/cardputerzero-system-shell/input_ascii.c' "$stage"
grep -q 'common/input_ascii.c' "$builder"
grep -q 'common/input_ascii.c' "$repo_root/image/build-image.sh"
...
grep -q '^SupplementaryGroups=cp0-wayland cp0-control cp0-display-control cp0-audio-control cp0-connectivity-control cp0-power-control cp0-provision-control cp0-usb-media-control$' "$shell_service"
