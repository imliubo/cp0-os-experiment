...
grep -q 'description: Release tag to build and upload' "$release_workflow"
grep -q 'github.event.release.tag_name || github.ref_name' "$release_workflow"
grep -q '^    runs-on: ubuntu-24.04-arm$' "$release_workflow"
...
grep -q 'native ARM64 runner; binfmt registration is unnecessary' "$release_workflow"
