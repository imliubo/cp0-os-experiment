...
- 遵循目标文件现有语言、格式和命名风格；默认使用 ASCII。维护中的 Markdown 文档遵循
  `docs/LOCALIZATION.zh-CN.md`：英文使用默认 `FILE.md`，简体中文使用配对的
  `FILE.zh-CN.md`。
...
实现改变产品行为、架构决策、构建输入或验收方法时，应同步更新最接近该事实的双语文档
和测试。不要在多个文档复制易漂移的状态；roadmap 记录完成条件，ADR 记录长期决策，Phase
