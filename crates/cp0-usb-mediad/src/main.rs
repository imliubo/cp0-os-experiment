...
use cp0_usb_mediad::{UsbMediaPaths, UsbMediaServer, UsbMediaService};

fn main() -> ExitCode {
    match run(env::args().skip(1).collect()) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("cp0-usb-mediad: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run(arguments: Vec<String>) -> Result<(), String> {
    let paths = UsbMediaPaths::default();
    match arguments.as_slice() {
        [command] if command == "serve" => {
            ...
            let mut service = UsbMediaService::new(paths);
            service.recover_status();
            UsbMediaServer::new(service, shell_uid)
                .serve(listener)
                .map_err(|error| error.to_string())
        }
        [command] if command == "emergency-stop" => UsbMediaService::new(paths)
            .emergency_stop()
            .map_err(|error| error.to_string()),
        _ => Err("usage: cp0-usb-mediad serve|emergency-stop".into()),
    }
}
...
fn systemd_listener() -> Result<UnixListener, String> {
    ...
    if listen_pid != std::process::id() || listen_fds != 1 || names != "usb-media" {
        return Err("one systemd USB media Unix listener is required".into());
    }
