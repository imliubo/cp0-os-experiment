...
use cp0_gpiod::{GpioServer, SysfsGpioBackend};

fn main() -> ExitCode {
    match run(env::args().skip(1).collect()) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("cp0-gpiod: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run(arguments: Vec<String>) -> Result<(), String> {
    match arguments.as_slice() {
        [command] if command == "serve" => {
            ...
            GpioServer::new(SysfsGpioBackend, [0])
                .serve(listener)
                .map_err(|error| error.to_string())
        }
        _ => Err("usage: cp0-gpiod serve".into()),
    }
}
...
fn systemd_listener() -> Result<UnixListener, String> {
    ...
    if listen_pid != std::process::id() || listen_fds != 1 || names != "gpio" {
        return Err("one systemd GPIO Unix listener is required".into());
    }
