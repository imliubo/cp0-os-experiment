...
use cp0_provisiond::{
    LinuxPlatformBackend, ProvisioningPaths, ProvisioningServer, ProvisioningService,
};
...
fn run(arguments: Vec<String>) -> Result<(), String> {
    ...
    let backend = LinuxPlatformBackend::new(paths.clone());
    ...
    match arguments.as_slice() {
        [command] if command == "serve" => {
            ...
            ProvisioningServer::new(service, shell_uid)
                .serve(listener)
                .map_err(|error| error.to_string())
        }
        [command] if command == "apply" => service
            .apply_boot_configuration()
            .map_err(|error| error.to_string()),
        _ => Err("usage: cp0-provisiond serve|apply".into()),
    }
}
...
fn systemd_listener() -> Result<UnixListener, String> {
    ...
    if listen_pid != std::process::id() || listen_fds != 1 || names != "provision" {
        return Err("one systemd provisioning Unix listener is required".into());
    }
