...
use cp0_radiod::{ConfiguredRadio, DEFAULT_RADIO_CONFIG, RadioServer, load_config};

fn main() -> ExitCode {
    match run(env::args().skip(1).collect()) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("cp0-radiod: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run(arguments: Vec<String>) -> Result<(), String> {
    match arguments.as_slice() {
        [command] if command == "serve" => {
            ...
            let config = load_config(DEFAULT_RADIO_CONFIG).map_err(|error| error.to_string())?;
            RadioServer::new(ConfiguredRadio::new(config), [0])
                .serve(listener)
                .map_err(|error| error.to_string())
        }
        _ => Err("usage: cp0-radiod serve".into()),
    }
}
...
fn systemd_listener() -> Result<UnixListener, String> {
    ...
    if listen_pid != std::process::id() || listen_fds != 1 || names != "radio" {
        return Err("one systemd radio Unix listener is required".into());
    }
