...
use cp0_displayd::{DisplayServer, SysfsBacklightBackend};

fn main() -> ExitCode {
    match run(env::args().skip(1).collect()) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("cp0-displayd: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run(arguments: Vec<String>) -> Result<(), String> {
    match arguments.as_slice() {
        [command] if command == "serve" => {
            ...
            DisplayServer::new(SysfsBacklightBackend::default(), [shell_uid])
                .serve(listener)
                .map_err(|error| error.to_string())
        }
        _ => Err("usage: cp0-displayd serve".into()),
    }
}
...
fn systemd_listener() -> Result<UnixListener, String> {
    ...
    if listen_pid != std::process::id() || listen_fds != 1 || names != "display" {
        return Err("one systemd display Unix listener is required".into());
    }
