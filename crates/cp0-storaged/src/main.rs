...
use cp0_storaged::{DEFAULT_STORAGE_ROOT, StorageServer, StorageService};

fn main() -> ExitCode {
    match run(env::args().skip(1).collect()) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("cp0-storaged: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run(arguments: Vec<String>) -> Result<(), String> {
    match arguments.as_slice() {
        ...
        _ => Err("usage: cp0-storaged serve".into()),
    }
}
...
fn systemd_listener() -> Result<UnixListener, String> {
    ...
    if listen_pid != std::process::id() || listen_fds != 1 || names != "storage" {
        return Err("one systemd storage Unix listener is required".into());
    }
