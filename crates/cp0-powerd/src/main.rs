...
use cp0_powerd::{PowerServer, SystemdPowerBackend};

fn main() -> ExitCode {
    match run(env::args().skip(1).collect()) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("cp0-powerd: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run(arguments: Vec<String>) -> Result<(), String> {
    match arguments.as_slice() {
        [command] if command == "serve" => {
            ...
            PowerServer::new(SystemdPowerBackend::default(), [shell_uid])
                .serve(listener)
                .map_err(|error| error.to_string())
        }
        _ => Err("usage: cp0-powerd serve".into()),
    }
}
...
fn systemd_listener() -> Result<UnixListener, String> {
    ...
    if listen_pid != std::process::id() || listen_fds != 1 || names != "power" {
        return Err("one systemd power Unix listener is required".into());
    }
